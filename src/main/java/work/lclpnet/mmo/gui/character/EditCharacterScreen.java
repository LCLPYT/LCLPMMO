...
public class EditCharacterScreen extends MMOScreen {
    ...
    protected void init() {
        ...
        this.saveButton = this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 144 + 5, 98, 20, new TranslationTextComponent("mmo.menu.edit_character.save"), (p_214308_1_) -> this.saveChanges()));
        this.addButton(new Button(this.width / 2 + 2, this.height / 4 + 144 + 5, 98, 20, new TranslationTextComponent("gui.cancel"), (p_214306_1_) -> this.minecraft.displayGuiScreen(prevScreen)));
        ...
        if (!character.getName().equals(name)) {
            ...
            LCLPNetwork.getAPI().renameCharacter(character.id, name).thenRun(() -> {
                ...
                if (APIException.NO_CONNECTION.equals(err)) {
                    ...
                    } else displayToast(new TranslationTextComponent("mmo.menu.edit_character.edit_failed"));
                } else {
                    displayToast(new TranslationTextComponent("mmo.menu.edit_character.edit_failed"));
                }
                return null;
