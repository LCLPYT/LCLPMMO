...
public class YoutubeDL {
    ...
    public static void download(String url, BiConsumer<Integer, File> handler) throws IOException {
        ...
        if (FFMPEG.isInPath())
            execYTDLCommand(consumer, url, "-o", dir.getAbsolutePath() + File.separatorChar + "%(title)s.%(ext)s", "-x");
        else
    ...
    public static void downloadQuery(String url, BiConsumer<Integer, File> handler) throws IOException {
        ...
        if (FFMPEG.isInPath())
            execYTDLCommand(consumer, s, "-o", dir.getAbsolutePath() + File.separatorChar + "%(title)s.%(ext)s", "-x");
        else
            execYTDLCommand(consumer, s, "-o", dir.getAbsolutePath() + File.separatorChar + "%(title)s.%(ext)s", "-x", "--ffmpeg-location", FFMPEG.getFFMPEGExecutable().getAbsolutePath());
    }
}
